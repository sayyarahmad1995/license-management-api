...
// applyAuthMiddleware applies authentication middleware
func (s *Server) applyAuthMiddleware(r chi.Router, tokenSvc service.TokenService) {
...
func (s *Server) initializeDependencies(userRepo repository.IUserRepository, licenseRepo repository.ILicenseRepository, activationRepo repository.ILicenseActivationRepository, auditRepo repository.IAuditLogRepository, legacyRL *middleware.RateLimiter, loginLimiter, registerLimiter, passwordResetLimiter, emailVerifyLimiter, licenseActivationLimiter *middleware.EnhancedRedisRateLimiter, redisClient interface{}) *ServiceDependencies {
	...
	var licenseCache *cache.LicenseCache
	if rc != nil {
		redisHost := config.GetConfigString("REDIS_HOST")
		if redisHost == "" {
			redisHost = "localhost"
		}
		redisPort := config.GetConfigString("REDIS_PORT")
		if redisPort == "" {
			redisPort = "6379"
		}
		redisAddr := fmt.Sprintf("%s:%s", redisHost, redisPort)
		redisPassword := config.GetConfigString("REDIS_PASSWORD")
		redisDB := config.GetConfigInt("REDIS_DB")
		licenseCache = cache.NewLicenseCache(redisAddr, redisPassword, redisDB, 5*time.Minute)
	} else {
		// Fallback: create cache that gracefully handles no Redis
		licenseCache = cache.NewLicenseCache("localhost:6379", "", 0, 5*time.Minute)
	}
	...
	if loginLimiter != nil {
		authHandler = handler.NewAuthHandlerWithEnhancedRateLimiter(authSvc, tokenSvc, loginLimiter, tokenRevocationSvc, tokenRotationSvc, notificationPreferenceSvc, notificationQueueSvc, validationSvc, emailSvc, sessionCacheSvc, auditSvc)
	} else {
		authHandler = handler.NewAuthHandler(authSvc, tokenSvc, legacyRL, tokenRevocationSvc, tokenRotationSvc, notificationPreferenceSvc, notificationQueueSvc, validationSvc, emailSvc, sessionCacheSvc, auditSvc)
	}

	licenseHandler := handler.NewLicenseHandler(licenseSvc, licenseRepo, machineFingerprintSvc, paginationSvc, dataExportSvc, bulkOperationSvc, licenseCache, auditSvc)
	healthHandler := handler.NewHealthHandler(s.db, circuitBreakerSvc, licenseCache)
	userHandler := handler.NewUserHandler(userRepo, licenseRepo, paginationSvc, dataExportSvc, auditSvc)
	auditHandler := handler.NewAuditHandler(auditRepo, userRepo, auditSvc)
...
func (s *Server) RegisterRoutes() http.Handler {
	...
	var loginLimiter, registerLimiter, passwordResetLimiter, emailVerifyLimiter, licenseActivationLimiter *middleware.EnhancedRedisRateLimiter

	if s.cfg.Env == "development" || s.cfg.Env == "dev" {
		s.log.Info("Rate limiting disabled in development environment")
		// All limiters remain nil
	} else {
		// Initialize enhanced Redis-based rate limiters for different endpoints (production only)
	...
	r.Route(APIv1, func(r chi.Router) {
		...
		r.Route("/auth", func(r chi.Router) {
			...
			r.Group(func(r chi.Router) {
				s.applyAuthMiddleware(r, deps.TokenSvc)
		...
		r.Group(func(r chi.Router) {
			...
			r.Route("/audit-logs", func(r chi.Router) {
				...
				r.Get("/export", deps.AuditHandler.ExportAuditLogs)
				r.Delete("/", deps.AuditHandler.DeleteAuditLogs)
			})
