...
func buildTestRouter(_ *testing.T, db *gorm.DB, cfg *config.JwtConfig, rateLimiter *middleware.RateLimiter) http.Handler {
	...
	// Initialize license cache for testing (use localhost Redis)
	licenseCache := cache.NewLicenseCache("localhost:6379", "", 0, 5*time.Minute)

	authHandler := handler.NewAuthHandler(authSvc, tokenSvc, rateLimiter, tokenRevocationSvc, tokenRotationSvc, notificationPreferenceSvc, notificationQueueSvc, validationService, emailSvc, sessionCacheSvc, auditSvc)
	...
	healthHandler := handler.NewHealthHandler(&testDatabaseService{db: db}, circuitBreakerSvc, licenseCache)
