...
func (m *MockTokenServiceExtended) GenerateRefreshToken(userID int) (string, error) {
	args := m.Called(userID)
	return args.String(0), args.Error(1)
}
...
func TestGenerateRefreshToken(t *testing.T) {
	...
	mockTokenSvc.On("GenerateRefreshToken", 1).Return("refresh_token_456", nil)

	token, err := mockTokenSvc.GenerateRefreshToken(1)
	assert.Nil(t, err)
	assert.NotEmpty(t, token)
}
