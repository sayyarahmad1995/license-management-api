...
func (lhs *licenseHeartbeatService) RecordHeartbeat(licenseKey string, machineFingerprint string) *errors.ApiError {
	...
	// Log audit event
	details := fmt.Sprintf("License heartbeat from machine: %s", machineFingerprint)
	lhs.auditSvc.LogAction("LICENSE_HEARTBEAT", "License", license.ID, &license.UserID, &details, nil)
