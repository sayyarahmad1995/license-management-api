...
func (as *authService) Register(req *dto.RegisterDto, ipAddress string) (*models.User, *errors.ApiError) {
	...
	as.auditSvc.LogAction("USER_REGISTERED", "User", user.ID, &user.ID, &details, &ipAddress)
...
func (as *authService) Login(req *dto.LoginDto, ipAddress string) (*dto.LoginResultDto, *errors.ApiError) {
	...
	refreshToken, errRT := as.tokenSvc.GenerateRefreshToken(user.ID, user.Email, user.Role)
	if errRT != nil {
		return nil, errors.NewInternalError("Failed to generate refresh token")
	}

	// Log audit
	details := fmt.Sprintf("User login successful from IP: %s", ipAddress)
	as.auditSvc.LogAction("USER_LOGIN", "User", user.ID, &user.ID, &details, &ipAddress)
...
func (as *authService) VerifyEmail(token string) *errors.ApiError {
	...
	as.auditSvc.LogAction("EMAIL_VERIFIED", "User", user.ID, &user.ID, &details, nil)
...
func (as *authService) RequestPasswordReset(email string) *errors.ApiError {
	...
	// Log audit
	details := fmt.Sprintf("Password reset requested for email: %s", user.Email)
	as.auditSvc.LogAction("PASSWORD_RESET_REQUESTED", "User", user.ID, &user.ID, &details, nil)
...
func (as *authService) ConfirmPasswordReset(token string, newPassword string) *errors.ApiError {
	...
	// Log audit
	details := "Password reset confirmed. New password set"
	as.auditSvc.LogAction("PASSWORD_RESET_CONFIRMED", "User", user.ID, &user.ID, &details, nil)
