...
func (h *AuthHandler) Register(w http.ResponseWriter, r *http.Request) {
	...
	// Log the user creation (registration)
	// For public registrations, we log with null userID. For admin-created users, userID would be set by the auth service
	...
	h.auditSvc.LogAction("USER_CREATED", "User", user.ID, nil, &detailsStr, &ipAddress)
...
func (h *AuthHandler) Login(w http.ResponseWriter, r *http.Request) {
	...
	var isAllowed bool = true // Default to allowed if no limiter is configured
	if h.enhancedRedisRateLimiter == nil {
		...
		// If all limiters are nil, isAllowed stays true (skip rate limiting)
	}
...
// getSameSiteMode returns SameSite mode based on SAME_SITE environment variable
// SAME_SITE=true: SameSite=Strict (maximum security, prevents cross-site cookies)
// SAME_SITE=false: SameSite=Lax (allows navigation links, easier for development)
// Default: Strict (if not set, defaults to true for security)
func getSameSiteMode() http.SameSite {
	sameSite := os.Getenv("SAME_SITE")
	// Default to "true" (Strict) if not specified for security
	if sameSite == "" {
		return http.SameSiteStrictMode
	}
	// Parse boolean value
	if sameSite == "true" || sameSite == "1" || sameSite == "yes" {
		return http.SameSiteStrictMode
	}
