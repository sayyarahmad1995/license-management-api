...
func NewLicenseHandler(licenseSvc service.LicenseService, licenseRepo repository.ILicenseRepository, machineFingerprintSvc service.MachineFingerprintService, paginationSvc service.PaginationService, dataExportSvc service.DataExportService, bulkOperationSvc service.BulkOperationService) *LicenseHandler {
...
func (h *LicenseHandler) GetLicenses(w http.ResponseWriter, r *http.Request) {
	...
	for _, license := range licenses {
		...
		if userIDStr != "" {
			if userID, err := strconv.Atoi(userIDStr); err == nil && license.UserID != userID {
				continue
			}
		}
