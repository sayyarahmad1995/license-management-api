...
func NewUserHandler(userRepo repository.IUserRepository, licenseRepo repository.ILicenseRepository, paginationSvc service.PaginationService, dataExportSvc service.DataExportService, auditSvc service.AuditService) *UserHandler {
	return &UserHandler{
		userRepo:      userRepo,
...
// @Description Retrieve a paginated list of all users (Admin only) with optional sorting and searching
// @Tags Users
// @Accept json
// @Produce json
...
// @Param sortBy query string false "Sort field: username, email, createdAt, role, status (default: none)"
...
func (h *UserHandler) GetUsers(w http.ResponseWriter, r *http.Request) {
	...
	if status != "" {
		var filtered []models.User
		for _, user := range users {
			if strings.EqualFold(string(user.Status), status) {
				filtered = append(filtered, user)
			}
		}
		users = filtered
...
func (h *UserHandler) UpdateUser(w http.ResponseWriter, r *http.Request) {
	...
	details := map[string]interface{}{
		...
		"fields_updated": map[string]interface{}{
			"notify_license_expiry":       user.NotifyLicenseExpiry,
			"notify_account_activity":     user.NotifyAccountActivity,
			"notify_system_announcements": user.NotifySystemAnnouncements,
		},
	}
	detailsJSON, _ := json.Marshal(details)
	detailsStr := string(detailsJSON)
	ipAddress := utils.GetClientIP(r)
	h.auditSvc.LogAction("USER_UPDATED", "User", user.ID, &currentUserID, &detailsStr, &ipAddress)
...
func (h *UserHandler) DeleteUser(w http.ResponseWriter, r *http.Request) {
	...
	user, _ := h.userRepo.GetByID(userID) // Get user again to capture details
	...
	detailsStr := string(detailsJSON)
	ipAddress := utils.GetClientIP(r)
	h.auditSvc.LogAction("USER_DELETED", "User", userID, &currentUserID, &detailsStr, &ipAddress)
...
func (h *UserHandler) UpdateUserStatus(w http.ResponseWriter, r *http.Request) {
	...
	oldStatus := user.Status // Capture old status for logging
	...
	detailsStr := string(detailsJSON)
	ipAddress := utils.GetClientIP(r)
	h.auditSvc.LogAction("USER_STATUS_CHANGED", "User", user.ID, &currentUserID, &detailsStr, &ipAddress)
