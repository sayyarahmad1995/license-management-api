...
Check the health status of the API, database, and Redis cache.
...
**Endpoint:** `GET /audit-logs?pageIndex=1&pageSize=20&action=LICENSE_CREATED&entityType=LICENSE`
...
- `action` (optional): Filter by action type (e.g., LICENSE_CREATED, USER_LOGIN)
- `entityType` (optional): Filter by entity type (e.g., LICENSE, USER)
  ...
  "data": [
    {
      ...
      "userId": 1,
      "username": "john_doe",
      "details": "License created for user ID 1. Max activations: 5, Expires: 2027-03-05",
...
**Endpoint:** `GET /audit-logs/user?user_id=1&pageIndex=1&pageSize=20`

Retrieve audit logs for a specific user (Admin only).

**Query Parameters:**
- `user_id` (required): The user ID to filter logs
- `pageIndex` (optional): Page number (default: 1)
- `pageSize` (optional): Items per page (default: 20, max: 100)
  ...
  "data": [
    {
      ...
      "userId": 1,
      "username": "john_doe",
      "details": "User login successful from IP: 192.168.1.100",
      "ipAddress": "192.168.1.100",
      "timestamp": "2026-03-05T14:20:00Z"
    }
  ],
...
**Endpoint:** `GET /audit-logs/export?format=csv&startDate=2026-03-01&endDate=2026-03-13&action=LICENSE_CREATED&userId=1`

Export filtered audit logs as CSV file (Admin only).

**Query Parameters:**
- `format` (optional): File format (currently supports "csv", default: csv)
- `startDate` (optional): Filter by start date (YYYY-MM-DD format)
- `endDate` (optional): Filter by end date (YYYY-MM-DD format)
- `action` (optional): Filter by action type
- `userId` (optional): Filter by user ID (admin-only filter)

**Response:**
```
CSV file download with columns:
ID, Action, Entity Type, Entity ID, User ID, Username, Details, IP Address, Timestamp
```

### Clear All Audit Logs (Admin Only)

**Endpoint:** `DELETE /audit-logs`

Delete all audit logs from the system (Admin only). This action cannot be undone.
...
| GET | `/audit-logs/user` | Get logs for specific user (Admin) | ✅ (Admin) |
| GET | `/audit-logs/stats` | Get audit log statistics (Admin) | ✅ (Admin) |
| GET | `/audit-logs/export` | Export logs as CSV (Admin) | ✅ (Admin) |
| DELETE | `/audit-logs` | Clear all logs (Admin) | ✅ (Admin) |

### Health Endpoints

| Method | Endpoint | Description | Auth Required |
|--------|----------|-------------|-----------------|
| GET | `/health` | Check API, database, and Redis health | ❌ |
...
### Use Case 6: Manage Audit Logs (Admin)

```bash
# 1. View audit logs with pagination
curl -X GET "http://localhost:8080/api/v1/audit-logs?pageIndex=1&pageSize=20" \
  -H "Authorization: Bearer <admin_access_token>"

# 2. Filter audit logs by action
curl -X GET "http://localhost:8080/api/v1/audit-logs?action=LICENSE_CREATED&entityType=LICENSE" \
  -H "Authorization: Bearer <admin_access_token>"

# 3. View logs for a specific user
curl -X GET "http://localhost:8080/api/v1/audit-logs/user?user_id=5" \
  -H "Authorization: Bearer <admin_access_token>"
...
# 5. Export audit logs as CSV with date range filtering
curl -X GET "http://localhost:8080/api/v1/audit-logs/export?startDate=2026-03-01&endDate=2026-03-13&format=csv" \
  -H "Authorization: Bearer <admin_access_token>" \
  -o audit_logs.csv

# 6. Clear all audit logs (irreversible action)
curl -X DELETE http://localhost:8080/api/v1/audit-logs \
  -H "Authorization: Bearer <admin_access_token>"
```

### Use Case 7: Check System Health

```bash
# Check API, database, and Redis status (no authentication needed)
curl -X GET http://localhost:8080/api/v1/health

# Response includes:
# - api_status: Overall API status
# - database: Database connectivity status
# - redis: Redis cache status
# - circuit_breakers: Status of internal circuit breakers
```
...
- Added comprehensive audit log management endpoints
  - List audit logs with filtering by action and entity type
  - View audit logs for specific users (admin-only)
  - Export audit logs as CSV with date range filtering
  - Clear/delete all audit logs (admin-only)
  - View audit log statistics (admin-only)
- Enhanced health check endpoint with Redis status
  - Added api_status field for overall system health
  - Added Redis connectivity monitoring
  - Improved circuit breaker status reporting
